...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const el = {
        ...
        techInterpretationSection: document.getElementById('techInterpretationSection'),
        techImageSlides: document.querySelectorAll('.tech-image-slide'),
        techButtons: document.querySelectorAll('.tech-button'),
        ...
        endingBackgroundSection: document.getElementById('endingBackgroundSection'),
     ...
     const SECTIONS = [
         ...
         '#subtitleSection',
         '#chapter1Section', 
         '#techInterpretationSection', // 技术解读页面
 ...
 // 添加新的第二章节过渡页面
         ...
         '#ending',
         '#endingBackgroundSection', // 添加结束前背景页面
    ...
    const fadeObserver = new IntersectionObserver((entries)=>{
     ...
     [
         ...
         el.chapter2PhotosSection, el.chapter3Section, el.chapter3PhotosSection, el.chapter4Section,
                  el.chapter4PhotosSection, el.analysisSection, el.techInterpretationSection, el.transitionSection, el.chapter2TransitionSection, el.customBackgroundSection, el.endingBackgroundSection,
    ...
    function setTechActive(index) {
        if (!el.techImageSlides?.length) return;
        techCurrentIndex = clamp(index, 1, techTotalSlides);
        el.techImageSlides.forEach((slide, i) => {
            slide.classList.toggle('active', i === techCurrentIndex - 1);
        });
        // 更新按钮状态
        if (el.techButtons?.length) {
            el.techButtons.forEach((button, i) => {
                button.classList.toggle('active', i === techCurrentIndex - 1);
            });
        }
    }

    // 技术解读页面滚动处理
    function handleTechScroll(delta) {
        if (!el.techInterpretationSection || !inViewport(el.techInterpretationSection)) return false;
        
        const atFirst = techCurrentIndex === 1 && delta < 0;
        const atLast = techCurrentIndex === techTotalSlides && delta > 0;
        ...
        if (atFirst || atLast) return false;
        
        if (throttleOnce(700)) return true;
        if (delta > 0 && techCurrentIndex < techTotalSlides) {
            setTechActive(techCurrentIndex + 1);
        } else if (delta < 0 && techCurrentIndex > 1) {
            setTechActive(techCurrentIndex - 1);
        }
        return true;
    }
...
function updateChartIframe(config) {
    ...
    if (el.techImageSlides?.length) {
        setTechActive(1);
        // 为技术按钮添加点击事件
        el.techButtons.forEach((button, index) => {
            button.addEventListener('click', () => {
                setTechActive(index + 1);
            });
        });
    }
...
// Chapter2页面自动播放背景音乐
function initChapter2BGM() {
    const chapter2Section = document.getElementById('chapter2PhotosSection');
    const chapter2BGM = document.getElementById('chapter2BGM');
    
    if (!chapter2Section || !chapter2BGM) {
        console.log('Chapter2页面或音频元素未找到');
        return;
    }
    
    // 创建Intersection Observer监听chapter2页面
    const chapter2Observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 页面进入视口，开始播放音乐
                chapter2BGM.play().catch(error => {
                    console.log('Chapter2音频播放失败:', error);
                    // 如果自动播放失败，尝试用户交互后播放
                    document.addEventListener('click', function playOnClick() {
                        chapter2BGM.play().catch(() => {});
                        document.removeEventListener('click', playOnClick);
                    }, { once: true });
                });
                console.log('Chapter2页面进入视口，开始播放背景音乐');
            } else {
                // 页面离开视口，停止播放音乐
                chapter2BGM.pause();
                console.log('Chapter2页面离开视口，停止播放背景音乐');
            }
        });
    }, {
        threshold: 0.3 // 当页面30%进入视口时触发
    ...
    chapter2Observer.observe(chapter2Section);
    console.log('Chapter2页面背景音乐监听器已设置');
}

// Chapter3页面自动播放背景音乐
function initChapter3BGM() {
    const chapter3Section = document.getElementById('chapter3PhotosSection');
    const chapter3BGM = document.getElementById('chapter3BGM');
    
    if (!chapter3Section || !chapter3BGM) {
        console.log('Chapter3页面或音频元素未找到');
        return;
    }
    
    // 创建Intersection Observer监听chapter3页面
    const chapter3Observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 页面进入视口，开始播放音乐
                chapter3BGM.play().catch(error => {
                    console.log('Chapter3音频播放失败:', error);
                    // 如果自动播放失败，尝试用户交互后播放
                    document.addEventListener('click', function playOnClick() {
                        chapter3BGM.play().catch(() => {});
                        document.removeEventListener('click', playOnClick);
                    }, { once: true });
                });
                console.log('Chapter3页面进入视口，开始播放背景音乐');
            } else {
                // 页面离开视口，停止播放音乐
                chapter3BGM.pause();
                console.log('Chapter3页面离开视口，停止播放背景音乐');
            }
        });
    }, {
        threshold: 0.3 // 当页面30%进入视口时触发
    ...
    chapter3Observer.observe(chapter3Section);
    console.log('Chapter3页面背景音乐监听器已设置');
}

// Chapter4页面自动播放背景音乐
function initChapter4BGM() {
    const chapter4Section = document.getElementById('chapter4PhotosSection');
    const chapter4BGM = document.getElementById('chapter4BGM');
    
    if (!chapter4Section || !chapter4BGM) {
        console.log('Chapter4页面或音频元素未找到');
        return;
    }
    
    // 创建Intersection Observer监听chapter4页面
    const chapter4Observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                // 页面进入视口，开始播放音乐
                chapter4BGM.play().catch(error => {
                    console.log('Chapter4音频播放失败:', error);
                    // 如果自动播放失败，尝试用户交互后播放
                    document.addEventListener('click', function playOnClick() {
                        chapter4BGM.play().catch(() => {});
                        document.removeEventListener('click', playOnClick);
                    }, { once: true });
                });
                console.log('Chapter4页面进入视口，开始播放背景音乐');
            } else {
                // 页面离开视口，停止播放音乐
                chapter4BGM.pause();
                console.log('Chapter4页面离开视口，停止播放背景音乐');
            }
        });
    }, {
        threshold: 0.3 // 当页面30%进入视口时触发
    ...
    chapter4Observer.observe(chapter4Section);
    console.log('Chapter4页面背景音乐监听器已设置');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(() => {
        ...
        initChapter2BGM(); // 初始化Chapter2页面背景音乐自动播放
        initChapter3BGM(); // 初始化Chapter3页面背景音乐自动播放
        initChapter4BGM(); // 初始化Chapter4页面背景音乐自动播放
    }, 500);
});
